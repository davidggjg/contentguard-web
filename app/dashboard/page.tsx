...
export default function Dashboard() {
  ...
  const [settings, setSettings] = useState<BlockSettings>({ blocked_domains: [], block_level: 'medium' })
  ...
  async function loadSettings(deviceId: string) {
    const { data } = await supabase
      ...
      .single()
    if (data) setSettings({ blocked_domains: data.blocked_domains || [], block_level: data.block_level || 'medium' })
    else setSettings({ blocked_domains: [], block_level: 'medium' })
  }
  ...
  return (
    <main className="min-h-screen p-4 max-w-2xl mx-auto">
      ...
      <div className="bg-slate-900 rounded-2xl p-4 mb-4">
        ...
        {showAddDevice && (
          <div className="flex gap-2 mb-4">
            <input
              ...
              placeholder="שם המכשיר (למשל: טלפון של דוד)"
              className="flex-1 bg-slate-800 rounded-xl px-3 py-2 text-white text-sm outline-none"
            />
            <button onClick={addDevice} className="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-xl text-sm">
              צור
            </button>
          </div>
        )}
        ...
        {devices.length === 0 ? (
          <p className="text-slate-500 text-center py-4">אין מכשירים עדיין. הוסף מכשיר ראשון!</p>
        ) : (
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-slate-400 text-xs mt-0.5">
                      קוד הפעלה: <span className="font-mono font-bold text-yellow-400">{device.activation_code}</span>
                    </p>
                  </div>
                  <div className={`w-2.5 h-2.5 rounded-full ${device.is_active ? 'bg-green-500' : 'bg-slate-600'}`} />
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="mb-4">
            ...
            <div className="flex gap-2 mb-2">
              <input
                ...
                className="flex-1 bg-slate-800 rounded-xl px-3 py-2 text-white text-sm outline-none"
              />
              <button onClick={addDomain} className="bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-xl text-sm">
                חסום
              </button>
            </div>
            <div className="flex flex-col gap-1 max-h-40 overflow-y-auto">
              {settings.blocked_domains.map(domain => (
                <div key={domain} className="flex items-center justify-between bg-slate-800 rounded-lg px-3 py-2">
                  ...
                  <button onClick={() => removeDomain(domain)} className="text-slate-500 hover:text-red-400 text-lg leading-none">
                    ×
                  </button>
                </div>
              ))}
              {settings.blocked_domains.length === 0 && (
                <p className="text-slate-600 text-sm text-center py-2">אין אתרים חסומים</p>
              )}
            </div>
          </div>
